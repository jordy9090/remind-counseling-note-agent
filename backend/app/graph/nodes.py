...
def _mock_structured_case(sanitized: SanitizedInput) -> StructuredCaseData:
    ...
    return StructuredCaseData(
        ...
        session_content=[
            EvidenceItem(
                content="이번 회기에서는 상담사 메모와 축어록을 바탕으로 주요 이슈, 사고 흐름, 다음 계획을 정리함.",
                evidence_type="mixed",
        ...
        nonverbal_observations=[
            EvidenceItem(
                content=sanitized.sources.nonverbal_notes or "비언어/반언어 정보는 입력되지 않음.",
                evidence_type="counselor_input" if sanitized.sources.nonverbal_notes else "needs_review",
                source_refs=["nonverbal_notes"] if sanitized.sources.nonverbal_notes else [],
            )
        ],
