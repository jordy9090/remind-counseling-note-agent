...
from app.schemas.evidence import (
    EvidenceSpanSelectionPayload, RetrievedTranscriptWindow, StoredTranscriptTurn,
)
from app.services import query_evidence_selection, raw_evidence_retrieval, transcript_windows
from app.services.query_evidence_selection import select_evidence_spans, select_evidence_spans_with_diagnostics
...
class RawWindowPipelineTests(unittest.TestCase):
    ...
    def test_local_rpc_excludes_other_user_and_case_windows(self):
        from scripts.evaluate_raw_window_selection import LocalWindowEvaluationStorage

        corpus = {
            "user_id": "u", "case_id": "c", "sessions": [{
                "session_id": "s1", "session_number": 1,
            }],
        }
        storage = LocalWindowEvaluationStorage(corpus)
        base = {
            "id": "own", "user_id": "u", "counselor_id": "u", "case_id": "c", "session_id": "s1",
            "start_turn_index": 0, "end_turn_index": 1, "source_ref": "transcript_window:s1:0-1",
            "window_text": "own", "embedding": [1.0, 0.0],
        }
        ...
        rows = storage.rpc("match_transcript_windows", {
            "query_embedding": [1.0, 0.0], "filter_user_id": "u", "filter_case_id": "c", "match_count": 12,
        })
    ...
    def test_selector_rejects_out_of_region_and_missing_intermediate_turn(self):
        ...
        selected, diagnostics = select_evidence_spans_with_diagnostics(
            query_text="query", region_turns=turns,
    ...
    def test_selector_allows_empty_evidence(self):
        self.assertEqual(select_evidence_spans(
            query_text="unsupported query", region_turns=make_turns(3), selector=lambda *_: {"spans": []},
        ), [])

    def test_selected_span_is_exact_raw_grounded_with_deterministic_source_ref(self):
        selected = select_evidence_spans(
            query_text="query", region_turns=make_turns(5), session_number=2,
        ...
        self.assertEqual(selected.evidence_text.splitlines(), [
            "[counselor] exact raw turn 1", "[client] exact raw turn 2", "[counselor] exact raw turn 3",
        ])
