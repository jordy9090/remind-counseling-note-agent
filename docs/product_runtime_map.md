...
| Browse owned cases | case_list in `pages/SessionDraftPage.tsx` (`CaseListWorkspace`, sidebar recent cases), `lib/caseList.ts` | GET `/api/cases` | `api/routes/cases.py::get_case_list` → `services/supabase_storage.py::list_cases`; wrapper `api/cases/list.py` (rewrite `/api/cases` → `/api/cases/list`) | Reads cases/sessions/generated_notes/document_exports/counseling_drafts with the user JWT + `user_id` filter; aggregates counts per case | T4 (`test_case_list.py`), T2 (wrapper), `pnpm verify:case-list` | Connected. Loaded on workspace entry and whenever the list screen opens; filter/search are client-side | Auth+DB. Not gated by R/P/G |
| Open a client dashboard / restore its records / update schedule | case_dashboard in `pages/SessionDraftPage.tsx` → `components/case-dashboard/CaseDashboardPanel.tsx` | GET `/api/cases/{case_id}/dashboard`, GET `/api/notes/drafts?case_id=`, PATCH `/api/cases/{case_id}/schedule`; record/draft restore reuses GET `/api/notes/records/{id}` and `/api/notes/drafts/{id}` | `api/routes/cases.py` → `services/supabase_storage.py`; wrapper `api/cases/dashboard.py` | Reads cases/sessions/generated_notes/document_exports; drafts by case; updates cases schedule | T4; `pnpm verify:case-list` (record grouping) | Connected. Case card → dashboard → per-session note/temporary-draft open; "새 회기 입력" pre-fills case ID/alias/next session number | Auth+DB. This path is not gated by R/P/G |
...
`vercel.json` rewrites draft-detail, record-detail, the case list (`/api/cases`), and case dashboard/schedule paths.
