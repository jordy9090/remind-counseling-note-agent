...
`vercel.json` rewrites draft-detail, record-detail, and every case path (`/api/cases`, dashboard, schedule, profile) onto the single `api/cases/dashboard.py` function. The deployment must stay at or under 12 serverless functions (Vercel Hobby limit); `test_case_list.py` guards this.
