...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Start from Landing | `components/AuthGate.tsx` → `pages/LandingPage.tsx`, `lib/supabase.ts` | Supabase Auth SDK anonymous sign-in | Supabase Auth; Re:mind API guard validates Bearer tokens afterward | Auth user/session; SDK maintains the auth session | No dedicated Landing/anonymous sign-in E2E; T2 checks API actors | Connected. Email/OAuth forms are disconnected from normal entry | FE URL/public key + remote anonymous sign-in enabled. Subsequent protected APIs require auth |
